...
public class Robot {
    ...
    public void update() {
        ...
        if (cycle % RobotConfig.TELEMETRY_CYCLE == 0) {
            updateTelemetry();
        ...
        if (cycle % RobotConfig.FREQ_CHECK_CYCLE == 0) {
            long update_duration = System.nanoTime()-last_update;
            update_freq = ((update_duration/(double)1000000000) * RobotConfig.FREQ_CHECK_CYCLE) != 0 ? (int)((1/(update_duration/(double)1000000000)) * RobotConfig.FREQ_CHECK_CYCLE) : Integer.MAX_VALUE;
    ...
    public void updateTelemetry() {
        ...
        if (warnings.size() > 0) {
            ...
            for (int i = 0; i < warnings.size(); i++) {
                addData(String.valueOf(i+1), warnings.get(i));
            }
        }

        addData("[RBT "+name+"]", components.size()+" components");
        addData("FREQ", update_freq);
    }
